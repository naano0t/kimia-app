...
if st.session_state.menu == "home":
    with st.container():
        ...
        try:
            ...
            st.markdown("<div style='text-align:center; font-size:12px; color:#999;'>[Logo panda tidak ditemukan]</div>", unsafe_allow_html=True)
...
# ===== MENU: KONVERSI =====
    ...
    kategori = st.selectbox("Pilih Kategori:", ["💧 Konsentrasi", "⚗ Molaritas", "📘 Normalitas"])
    ...
    st.button("⬅ Kembali ke Menu Utama", on_click=lambda: st.session_state.update({"menu": "home"}))

# ===== MENU: TABEL PERIODIK =====
elif st.session_state.menu == "tabel":
    st.markdown("<h2>📘 Tabel Periodik Sederhana</h2>", unsafe_allow_html=True)
    st.image("https://upload.wikimedia.org/wikipedia/commons/0/01/Periodic_table_large.png", caption="Tabel Periodik Unsur")
    st.button("⬅ Kembali ke Menu Utama", on_click=lambda: st.session_state.update({"menu": "home"}))
